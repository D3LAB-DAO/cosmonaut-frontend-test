...
import L0C2 from "../chapter2/L0C2";
import L0C1U1 from "./unit/L0C1U1";

const Section = tw.section`fixed h-full bottom-0 w-full z-50 bg-black bg-opacity-80 justify-center items flex`;
...
function L0C1Start() {
  ...
  console.log(checkChID);
  console.log(checkId);

  const navigate = useNavigate();
  function handleStart(e) {
    e.preventDefault();
    navigate(`/lesson/${id}/chapter/${chID}/unit/1`);
  }
              ...
              {info?.map(e => (
                <Button>
                  <div class="block w-full h-full md:p-6 p-4 items-center divide-y-2 divide-indigo-900 justify-center">
                    <ChNumber>Chapter {e.chapter}</ChNumber>
